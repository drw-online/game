...
來源
--------------------------------------------------------------------------
  5.技能圖片/技能書對照表.csv          1100 本技能書 (物品ID / 階級 / 技能代號 / 上限 / 職業 / 圖示)
...
★ 圖示檔名用 技能書對照表 的「圖示」欄, 不用 技能圖示對應表 的「圖片檔名」。
  ...
  圖(ba_dissonance / cg_moonlit), 1100 支一個都不缺。

★ 中文名稱有兩支在 技能圖示對應表 是空的(BA_FROSTJOKER / LK_CONCENTRATION),
  退回用 技能書對照表 的物品名稱去掉「技能書」三個字。

★ 洋紅 (255,0,255) 是去背色, 1098 個圖示裡有 828 個用它。其餘 270 個本來
...
# ------------------------------------------------------------------ 來源
def load_books():
    """技能書對照表 -> 1100 筆, 保持 CSV 原順序(= 物品 ID 順序)。"""
